...
//                The aforementioned shoulders are rounded body parts that aid arms' flexibility. One's back is found on the opposite side of the stomach, and is a flat section of the body that contains important muscles that're intended to protect the lungs and other internal organs, in addition to helping humans perform certain motions and actions.""";
        String[] sentences = initial_text.split("[/.!?]\\s*");
        Map<String, Set<Integer>> invert_index = new HashMap<>();
        for (int i = 0; i < sentences.length; i++) {
            String[] words = sentences[i].replaceAll("[,\"():]", "").split("\\s");
            for (String word: words) {
                if (invert_index.get(word) == null) {
                    Set<Integer> set = new TreeSet<>();
                    invert_index.put(word, set);
                }
                invert_index.get(word).add(i);
            }
        }
        Set<Integer> max_set = new TreeSet<>();
        StringBuilder frequent_word = new StringBuilder();
        for(Map.Entry<String, Set<Integer>> curr: invert_index.entrySet()) {
            if (curr.getValue().size() > max_set.size()) {
                max_set = curr.getValue();
                frequent_word = new StringBuilder(curr.getKey());
            } else if (curr.getValue().size() == max_set.size() && curr.getValue().equals(max_set)) {
                frequent_word.append(", ").append(curr.getKey());
            }
        }
        System.out.println("Самые частые слова: " + frequent_word);
        for (int index: max_set) {
            System.out.println(index + ": " + sentences[index]);
        }
    }
}
